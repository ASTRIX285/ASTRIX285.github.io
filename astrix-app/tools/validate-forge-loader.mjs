...
assert.match(artifactSelectionRuntime,/const next=\{\.\.\.state,workingBuild:clone\(state\.workingBuild\)\}/,'Artifact recommendation must copy only Working Build and retain the immutable Original Build.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260904-elemental-options-loader-1/,'Build Forge must load the elemental option and blank AI mod-canvas sequence without stale browser code.');
