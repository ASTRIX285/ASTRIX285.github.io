...
assert.match(html,/forge-loader\.mjs\?v=20260905-weapon-audit-1/,'Forge Loader must load the memory-safe top-50 scanner without a stale browser module.');
...
assert.match(buildRuntime,/writeState\(applied\.state,\{memoryOnly:true\}\)/,'An expanded atomic transfer must stay in memory instead of being serialized into duplicate Web Storage copies.');
...
assert.match(buildRuntime,/if\(volatileStateMemoryOnly\)return true;/,'Working Build updates must preserve the memory-only mode established by the atomic transfer.');
...
assert.match(buildRuntime,/isProtectedForgeTransfer=[\s\S]*?current\?\.workingBuild\?\.forgeLoaderDecision[\s\S]*?currentBinding\.characterId===String\(detail\.characterId\);if\(isProtectedForgeTransfer\)return/,'A later live-profile event must not overwrite the already validated Forge Loader Working Build.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260905-manual-editor-2/,'Build Forge must load the complete manual-editor and guarded-Apply release without stale browser code.');
...
assert.match(perkPlanRuntime,/const requirements=\[[\s\S]*?'captureSnapshot'[\s\S]*?'transferItems'[\s\S]*?'equipItems'[\s\S]*?'verifyEquipment'[\s\S]*?'insertSocketPlugFree'[\s\S]*?'verifyFinalState'/,'The complete live-transfer plan must preserve fresh snapshot, transfer, equip, free-socket and final-verification phases.');
assert.match(perkPlanRuntime,/kind:'destiny-complete-loadout-transfer'[\s\S]*?requiresUserConfirmation:true[\s\S]*?executionPolicy:'fresh-read-transfer-equip-free-sockets-final-readback'/,'The complete transfer must remain staged behind explicit confirmation and a fresh-read execution policy.');
assert.match(buildRuntime,/function buildLivePlan\(\)[\s\S]*?createLiveTransferPreflight\(build\)[\s\S]*?createLiveTransferPlan\([\s\S]*?function openApplyConfirmation\(\)[\s\S]*?pendingApplyPlan=plan[\s\S]*?async function executeConfirmedApply\(\)[\s\S]*?executeLiveTransferPlan\(confirmLiveTransferPlan\(plan\)/,'Build Forge must stage, display and finally confirm the same exact live-transfer plan.');
assert.match(liveActionsRuntime,/status!==['"]confirmed['"][\s\S]*?Final user confirmation is required[\s\S]*?requestFreshProfile[\s\S]*?freshTransferSteps\(plan,fresh\)[\s\S]*?freshSocketChanges\(plan,fresh\)[\s\S]*?\/bungie\/actions\/transfer-item[\s\S]*?\/bungie\/actions\/equip-items[\s\S]*?\/bungie\/actions\/socket-plug-free[\s\S]*?finally\{[\s\S]*?requestFreshProfile/,'The executor must reject unconfirmed plans, revalidate exact live evidence, run transfer/equip/socket order, and always read back final state.');
