...
assert.match(runtime,/if\(!selectionStored&&!transferStored\)[\s\S]*?No build was changed/,'Forge Loader may navigate only when the staged selection has a complete browser or IndexedDB transfer path.');
...
assert.match(html,/forge-loader\.mjs\?v=20260904-atomic-forge-transfer-1/,'Forge Loader must load the atomic staged-armour handoff without a stale browser module.');
assert.match(runtime,/forge-loader-build-handoff\.mjs\?v=20260904-atomic-forge-transfer-1/,'Forge Loader must refresh the protected baseline writer with the atomic transfer release.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260904-atomic-forge-transfer-1/,'Build Forge must load the atomic transfer recovery without stale browser code.');
