...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260903-forge-intelligence-1/,'Build Forge must load the responsive evidence-ranked handoff, recommendation review and shared command header without stale browser code.');
...
assert.match(css,/\.forge-loader-workspace\{[^}]*grid-template-columns:var\(--apx-workspace-columns,minmax\(360px,20%\) minmax\(720px,1fr\) minmax\(420px,24%\)\)/,'Wide Forge Loader must use the shared left, centre and right proportions.');
