...
assert.match(liveActionsRuntime,/status!==['"]confirmed['"][\s\S]*?Final user confirmation is required[\s\S]*?requestFreshProfile[\s\S]*?characterActivityRestriction\(plan,fresh\)[\s\S]*?freshTransferSteps\(plan,fresh\)[\s\S]*?freshSocketChanges\(plan,fresh\)[\s\S]*?\/bungie\/actions\/transfer-item[\s\S]*?\/bungie\/actions\/equip-items[\s\S]*?phase:'verify-equipment'[\s\S]*?verifyEquippedItems\(plan,equippedProfile\)[\s\S]*?weapon-sockets[\s\S]*?armour-mods[\s\S]*?finally\{[\s\S]*?requestFreshProfile/,'The executor must reject unconfirmed plans, enforce component-204 activity state, verify fresh equipped items, split socket phases, and always read back final state.');
