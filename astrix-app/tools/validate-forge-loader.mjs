...
assert.match(buildRuntime,/shouldReplaceBuildState\(current,detail,\{vaultSelection:params\.get\('vault'\)==='selection',explicitlySelectedCharacterId\}\);[\s\S]*?if\(!replace\)return/,'A background live-profile event must not overwrite the already validated Forge Loader Working Build.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260905-generate-state-2/,'Build Forge must load the generated-state correction without stale browser code.');
