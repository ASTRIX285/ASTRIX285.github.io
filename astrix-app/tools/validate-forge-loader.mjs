...
assert.equal(writeForgeLoaderBuildSnapshot(oversizedProfileBuild,binding,{stores:[handoffStore]}),true,'The protected direct-entry baseline must fit browser storage after non-evaluator profile payloads and stale Build Forge caches are removed.');
...
assert.match(runtime,/if\(!baselineStored\)\{[\s\S]*?Build Forge will recover the protected Original Build directly from Bungie\.[\s\S]*?\}\s*const selected=/,'A browser storage quota failure must continue through authenticated Bungie recovery instead of blocking the Forge Loader handoff.');
assert.doesNotMatch(runtime,/if\(!baselineStored\)\{[^}]*?return;/,'A rejected browser baseline must not prevent the verified staged armour from reaching Build Forge.');
assert.match(runtime,/if\(!baselineStored\)url\.searchParams\.set\('baseline','bungie-recovery'\)/,'The destination must receive an explicit authenticated baseline recovery signal when Web Storage rejects the snapshot.');
...
assert.match(html,/forge-loader\.mjs\?v=20260903-compact-selection-1/,'Forge Loader must load the compact staged-armour handoff without a stale browser module.');
assert.match(runtime,/forge-loader-build-handoff\.mjs\?v=20260903-storage-recovery-1/,'Forge Loader must refresh the protected baseline writer with the authenticated recovery release.');
...
assert.match(buildRuntime,/vault-selection-state\.mjs\?v=20260903-compact-selection-1/,'Build Forge must refresh the compact selection reader and fallback-store recovery.');
...
assert.match(runtime,/releaseGuardianSessionStorageFallbacks\(\);selectionStored=writeVaultArmourSelection\(selection\)/,'A failed compact selection write must release only recoverable profile fallbacks and retry once.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260903-compact-selection-1/,'Build Forge must load the compact selection and authenticated baseline recovery state without stale browser code.');
...
assert.match(buildRuntime,/window\.confirm\([\s\S]*?confirmPerkChangePlan\(plan\)[\s\S]*?applyConfirmedPerkChangePlan\(confirmed\)/,'Any live Build Forge action must remain behind explicit user confirmation.');
