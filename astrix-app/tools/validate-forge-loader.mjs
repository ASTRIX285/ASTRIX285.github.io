...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260905-forge-responsive-1/,'Build Forge must load the complete verified weapon recommendation sequence without stale browser code.');
...
assert.match(buildRuntime,/async function initialiseBuildForge\(\)[\s\S]*?await restoreAtomicForgeTransfer\(\)[\s\S]*?atomicTransfer\|\|readState\(\)[\s\S]*?finally\{\s*render\(\);\s*\}/,'The initial Forge Loader handoff must restore its atomic record and release the 58% gate without automatically staging an Artifact recommendation.');
...
assert.match(perkPlanRuntime,/function createLiveTransferPlan[\s\S]*?key:"snapshot"[\s\S]*?key:"equip"[\s\S]*?key:"verify-equipment"[\s\S]*?key:"weapon-perks"[\s\S]*?key:"armour-mods"[\s\S]*?key:"artifact"[\s\S]*?key:"verify-final"/,'The complete live-transfer plan must preserve the equip-first, socket-second and final-verification order.');
assert.match(perkPlanRuntime,/kind:"destiny-complete-loadout-transfer"[\s\S]*?requiresUserConfirmation:true[\s\S]*?executionPolicy:"equip-then-mutate-sockets-then-verify"/,'The complete transfer must remain staged behind explicit user confirmation.');
assert.match(buildRuntime,/async function applyBuild\(\)[\s\S]*?createLiveTransferPreflight\([\s\S]*?createLiveTransferPlan\([\s\S]*?if\(!plan\.ready\)[\s\S]*?No live Guardian changes were made/,'Build Forge must block all partial live mutation until the complete authenticated route passes preflight.');
