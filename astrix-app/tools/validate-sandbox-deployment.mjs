...
const authRecord=read('astrix-auth-worker/src/auth-record.ts');
...
assert.doesNotMatch(`${authWorker}\n${guardianAuth}\n${pagesWorker}`,/playstation|sony|psnid|\bpsn\b/i,'ASTRIX authentication must never select or embed a PlayStation or Sony route.');
assert.doesNotMatch(`${authWorker}\n${guardianAuth}\n${pagesWorker}`,/\b(?:xbox|steam|twitch)\b|epic games/i,'ASTRIX authentication must never select or embed any linked-platform provider route.');
assert.match(authWorker,/const destinyMemberships: Membership\[\] = \(membershipData\.Response\?\.destinyMemberships \|\| \[\]\)\.map\(\(m\) => \(\{[\s\S]*?membershipType: m\.membershipType,[\s\S]*?membershipId: m\.membershipId,[\s\S]*?const primaryMembershipId = membershipData\.Response\?\.primaryMembershipId \|\| null;[\s\S]*?destinyMemberships\.find\(\(m\) => m\.membershipId === primaryMembershipId\) \|\| destinyMemberships\[0\] \|\| null;/,'ASTRIX must accept Bungie-linked memberships without platform-specific branching and follow Bungie’s primary membership selection.');
...
assert.match(authWorker,/INTERNAL_ACCESS_HOST = "astrix-auth\.internal"[\s\S]*?\/internal\/access\/recovery-ticket[\s\S]*?\/session\/recover/,'Recovery tickets must cross only the private auth bridge before the public cookie restoration endpoint.');
