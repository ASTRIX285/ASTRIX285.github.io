...
assert.match(buildHtml,/<section class="design-section armour-design-section gear-combined"[\s\S]*?<div class="eq-head"><h3 id="buildArmourTitle">ARMOUR & MODS<\/h3><span class="tools">VERIFIED WORKING BUILD<\/span><\/div>[\s\S]*?<div class="gear-subhead"><span>Armour above · 6 functional mod slots below<\/span><span>Hover any sourced icon for Bungie details<\/span><\/div>[\s\S]*?id="armourGrid" class="gear-columns"/,'Build Armour must duplicate the locked Character card renderer without exposing internal T5 and masterwork gate text');
...
assert.match(buildHtml,/paradox-build-space\.mjs\?v=20260903-storage-recovery-1/,'Build must load the authenticated baseline recovery runtime without stale code');
