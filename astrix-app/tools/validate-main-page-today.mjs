...
assert.match(buildHtml,/<section class="design-section armour-design-section gear-combined"[\s\S]*?<div class="eq-head"><h3 id="buildArmourTitle">ARMOUR & MODS<\/h3><span class="tools">VERIFIED WORKING BUILD<\/span><\/div>[\s\S]*?<div class="gear-subhead"><span>T5 armour above · masterwork \+ 2 general \+ 3 armour mod slots below<\/span><span>Hover any sourced icon for Bungie details<\/span><\/div>[\s\S]*?id="armourGrid" class="gear-columns"/,'Build Armour must duplicate the locked Character card renderer while explaining its T5 and socket contract');
...
assert.match(buildHtml,/<aside class="build-right-rail"[\s\S]*?<section class="panel recommendation-panel"[\s\S]*?<section class="panel validation-panel design-section test-loop"[\s\S]*?<section class="panel intelligence" data-paradox-analysis>/,'Recommendations, Validation Loop and Intelligence must use the right rail.');
...
assert.match(buildHtml,/paradox-build-space\.css\?v=20260903-complete-build-forge-1/,'Build must load the complete three-column Build Forge presentation without a stale cache');
assert.match(buildHtml,/paradox-build-space\.mjs\?v=20260903-complete-build-forge-1/,'Build must load the complete verified recommendation runtime without stale code');
