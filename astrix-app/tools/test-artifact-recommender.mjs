...
const bad=resolveArtifactByProvenance(livePayload,'cid-missing');if(bad.state!=='state-unavailable'||bad.activePerks.length)throw new Error('bad character fallback '+JSON.stringify(bad));
const shared=resolveArtifactByProvenance({profile:{profileProgression:{data:{seasonalArtifact:{artifactHash:999}}}},artifactDefinition:{hash:999,displayProperties:{name:'Test Artifact'}}},'cid-live');if(shared.state!=='state-unavailable')throw new Error('share provenance failed '+JSON.stringify(shared));
