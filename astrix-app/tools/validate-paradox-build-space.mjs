...
import {composeForgeRecommendationAsync,composeForgeRecommendation,filterExoticCompatibleSubclasses} from '../pages/guardian-workspace-v2/paradox-build-space/paradox-forge-intelligence.mjs';
...
assert.match(runtime,/await composeForgeRecommendationAsync\(\{build:forgeComputationProjection\(working\),candidate,element:selectedRecommendationElement,analyzeBuild:analyzeForgeCandidate\}\)/,'Generation must compare verified subclass sockets through a memory-bounded deterministic Forge intelligence projection.');
...
const sourceBefore=JSON.stringify(intelligenceSource),candidateBefore=JSON.stringify(intelligenceCandidate);
let ticks=0;const heartbeat=setInterval(()=>ticks++,0);
const cooperative=await composeForgeRecommendationAsync({build:intelligenceSource,candidate:intelligenceCandidate,element:'arc',analyzeBuild:analyseCandidate},{budgetMs:0});
clearInterval(heartbeat);
assert.ok(ticks>0,'Generation must yield to user input and paint during candidate search');
assert.deepEqual(cooperative,composed,'Cooperative scheduling must preserve deterministic choices and evidence');
assert.equal(JSON.stringify(intelligenceSource),sourceBefore,'Generation must not mutate the input snapshot');
assert.equal(JSON.stringify(intelligenceCandidate),candidateBefore,'Branch search must not mutate catalogue selections');
assert.doesNotMatch(runtime.slice(runtime.indexOf('async function initialiseBuildForge()')),/applyForgeArtifactRecommendation|refreshForgeArtifactRecommendation|composeForgeRecommendation/,'Entry must retain Artifact and Super until an explicit user request');
console.log('FORGE_COOPERATIVE_GENERATION=PASS');

const {runInNewContext}=await import('node:vm');
globalThis.document={addEventListener(){},dispatchEvent(){return true;},querySelector(){return null;},querySelectorAll(){return [];},getElementById(){return null;}};
const {analyzeLiveGuardian}=await import('../pages/guardian-workspace-v2/guardian-paradox-live-adapter.mjs');
const projectionStart=runtime.indexOf('const FORGE_ANALYSIS_FIELDS=');
const projectionEnd=runtime.indexOf('function mergeComposedRecommendation',projectionStart);
const projectionScope={analyzeLiveGuardian};
runInNewContext(runtime.slice(projectionStart,projectionEnd)+'this.projectAnalysis=analyzeForgeCandidate;',projectionScope);
const analysisFixture={...intelligenceSource,...composed.workingBuild,unusedInventory:'x'.repeat(100000)};
assert.deepEqual(projectionScope.projectAnalysis(analysisFixture),analyzeLiveGuardian(analysisFixture),'Search projection must retain all directed-loop analysis evidence');
let rendered=0,writes=0;
const entryScope={restoreAtomicForgeTransfer:async()=>state,readState:()=>state,applyPendingVaultSelection:s=>s,writeState:()=>writes++,render:()=>rendered++,console};
runInNewContext(runtime.slice(runtime.indexOf('async function initialiseBuildForge()')).replace('void initialiseBuildForge();','this.entry=initialiseBuildForge;'),entryScope);
await entryScope.entry();
assert.equal(rendered,1);assert.equal(writes,0,'Handoff entry must not replace the transferred Super or Artifact');
console.log('FORGE_ENTRY_AND_ANALYSIS_PROJECTION=PASS');
