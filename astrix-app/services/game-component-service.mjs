/** Read-only service for manifest-backed Paradox Forge game components. */
...
export class GameComponentService {
  ...
  async load(options = {}) {
    ...
    if (!response.ok) throw new Error(`Unable to load game components: ${response.status}`);
  ...
  buildIndexes() {
    for (const index of [this.byHash, this.byId, this.byName, this.byType, this.byClass, this.bySubclass]) index.clear();
  ...
  ensureLoaded() {
    if (!this.loaded) throw new Error('Game-component catalogue has not been loaded.');
  }
  ...
  getByName(name) { this.ensureLoaded(); return [...(this.byName.get(normalise(name)) ?? [])]; }
  getByType(type) { this.ensureLoaded(); return [...(this.byType.get(normalise(type)) ?? [])]; }
  getByClass(className) { this.ensureLoaded(); return [...(this.byClass.get(normalise(className)) ?? [])]; }
  getBySubclass(subclass) { this.ensureLoaded(); return [...(this.bySubclass.get(normalise(subclass)) ?? [])]; }
  getArtifacts() { this.ensureLoaded(); return [...this.catalogue.artifacts]; }
  ...
  search(query, filters = {}) {
    ...
    return this.catalogue.components.filter((component) => {
      ...
      const text = [component.name, component.componentType, component.class, component.subclass,
        component.officialDescription, ...(component.curated?.effects ?? []),
        ...(component.curated?.inputs ?? []), ...(component.curated?.outputs ?? [])]
        .map(normalise).join(' ');
