/**
 * ASTRIX PARADOX - ARMOUR CATALOGUE SERVICE
 * Read-only curated armour catalogue resolver and manifest indexer for Paradox Forge.
 */
...
export class ArmorService {
  ...
  async load(options = {}) {
    ...
    if (typeof this.fetchImplementation !== 'function') {
      throw new Error('ArmorService requires fetch.');
    }
    const response = await this.fetchImplementation(url, { cache: 'no-store' });
    if (!response.ok) {
      throw new Error(`Unable to load armour catalogue: ${response.status}`);
    }
  ...
  setCatalogue(catalogue) {
    if (!catalogue || !Array.isArray(catalogue.armor)) {
      throw new Error('Armour catalogue must contain an armor array.');
    }
  ...
  ensureLoaded() {
    if (!this.loaded || !this.catalogue) {
      throw new Error('Armour catalogue has not been loaded.');
    }
  }

  getAllArmor() {
    this.ensureLoaded();
    return [...this.catalogue.armor];
  }
  ...
  getByClass(className) {
    this.ensureLoaded();
    return [...(this.byClass.get(normalize(className)) ?? [])];
  }

  getBySlot(slot) {
    this.ensureLoaded();
    return [...(this.bySlot.get(normalize(slot)) ?? [])];
  }

  getByRarity(rarity) {
    this.ensureLoaded();
    return [...(this.byRarity.get(normalize(rarity)) ?? [])];
  }

  getBySetName(setName) {
    this.ensureLoaded();
    return [...(this.bySetName.get(normalize(setName)) ?? [])];
  }
  ...
  search(query, filters = {}) {
    ...
    return this.catalogue.armor.filter((item) => {
      ...
      const stats = (item.official?.investmentStats ?? [])
      ...
      const haystack = normalize([
        ...
        item.curated?.setName,
        item.curated?.usageNotes,
        ...(item.curated?.setTags ?? []),
        stats
      ].join(' '));
