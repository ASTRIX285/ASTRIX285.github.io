...
function syncEquippedSubclass(detail={}){ingestSubclassCatalog(detail);const element=String(detail.subclass||'').trim().toLowerCase();const characterClass=String(detail.characterClass||'Guardian').trim();const rawCatalog=Array.isArray(detail.subclassCatalog)?detail.subclassCatalog:[];const catalog=manifestElementCatalog(rawCatalog);const identity=catalog.find(item=>elementKey(item)===element)||detail.subclassIdentity||null;const icon=manifestElementIcon(identity)||subclassIconCache.get(subclassCacheId(characterClass.toLowerCase(),element))||'';const picker=document.getElementById('subclassPicker');renderEquippedSubclass({root:document.getElementById('equippedSubclassSummary'),iconNode:document.getElementById('equippedSubclassIcon'),nameNode:document.getElementById('equippedSubclassName'),metaNode:document.getElementById('equippedSubclassMeta'),subclass:element,subclassName:detail.subclassName||element,characterClass,icon});renderSubclassPicker({root:picker,characterClass,subclass:element,subclassOptions:catalog,onSelect:item=>{const nextElement=elementKey(item);const nextBuild=item?.subclassBuild||item?.build||{};const next={...detail,subclass:nextElement,subclassName:item?.name||item?.displayName||nextElement,subclassIcon:manifestElementIcon(item),subclassBuild:nextBuild,super:nextBuild.super||null,superOptions:nextBuild.superOptions||[]};syncEquippedSubclass(next);populateSuperChain(next);}});applyManifestElementIcons(picker,catalog);}
