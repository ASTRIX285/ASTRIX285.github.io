...
import {applyForgeArtifactRecommendation} from './paradox-artifact-selection.mjs?v=20260904-artifact-weapon-synergy-1';
...
import {recommendArmourMods,selectOwnedWeapons} from './paradox-loadout-intelligence.mjs?v=20260904-owned-weapon-synergy-1';
...
function renderRecommendedBuildReview(build={}){
  ...
  byId('recommendedArtifactSummary').innerHTML=artifact?`<div class="review-artifact-identity">${reviewIcon(artifact,'Artifact')}<div><b>${esc(artifact.name||'VERIFIED ARTIFACT')}</b><span>${artifactReady?`PARADOX BEST VERIFIED FIT · ${Number(recommendation.totalScore||0)} SYNERGY SCORE`:'EVIDENCE LIMITED · CURRENT CONFIGURATION SHOWN'}</span></div></div><div class="review-artifact-perks">${perks.map(perk=>reviewIcon(perk,'Artifact perk')).join('')||'<small>NO LEGAL VERIFIED PERK CHANGE RESOLVED</small>'}</div><div class="review-artifact-synergy"><b>ARTIFACT SYNERGY</b><ul>${artifactSynergyRows}</ul></div>`:'<small>ARTIFACT STATE UNAVAILABLE</small>';
...
async function generateMaxLoadout(){
  ...
  try{
    ...
    next=protectBuildState({...next,workingBuild:working});const artifactResult=applyForgeArtifactRecommendation(next,{currentSeasonNumber,force:true});next=artifactResult.state;working=JSON.parse(JSON.stringify(next.workingBuild));
