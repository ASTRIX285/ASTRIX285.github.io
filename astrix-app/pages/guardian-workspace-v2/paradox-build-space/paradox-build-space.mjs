...
import {applyVaultArmourSelection,clearVaultArmourSelection,readVaultArmourSelection,validateVaultArmourSelection} from '../../vault/vault-selection-state.mjs?v=20260904-memory-safe-transfer-1';
import {applyForgeArtifactRecommendation} from './paradox-artifact-selection.mjs?v=20260904-memory-safe-transfer-1';
...
import {recommendArmourMods,selectOwnedWeapons} from './paradox-loadout-intelligence.mjs?v=20260903-loadout-intelligence-1';
...
function artifactRecommendationMarkup(build){
  ...
  return '<div class="artifact-recommendation is-ready"><div><b>PARADOX BEST VERIFIED FIT</b><span>'+artifactChoice+' · '+Number(recommendation.selectedMatchedCount||0)+' OF '+Number(recommendation.selectionLimit||0)+' PICKS DIRECTLY MATCH</span></div>'+(reasons?'<ol>'+reasons+'</ol>':'')+'<small>Artifact 2.0 is ranked from its verified socket buckets, then each bucket is filled from the staged Exotic, stat priorities, armour-set traits, subclass/Super and weapons. Working Build only · live Guardian unchanged.</small><button type="button" data-artifact-recommend>RECALCULATE BEST FIT</button></div>';
}
...
function renderRecommendationControls(build={}){
  const supported=new Map(resolvedSubclassOptions(build).filter(hasVerifiedSubclassSockets).map(item=>[elementOf(item),item]).filter(([element])=>BUILD_ELEMENTS.includes(element)));
  ...
  document.querySelectorAll('[data-recommendation-element]').forEach(button=>{const element=button.dataset.recommendationElement,available=hasDecision&&supported.has(element),selected=available&&element===selectedRecommendationElement;button.disabled=!available||recommendationBusy;button.classList.toggle('is-selected',selected);button.setAttribute('aria-pressed',String(selected));button.title=!hasDecision?'Stage a verified Forge Loader armour result first.':available?`Evaluate a verified ${element} damage build with the staged armour result.`:`No verified ${element} build option is available for this Guardian.`;});
  ...
  const armourValidation=validateTierFiveArmour(build),hasElement=Boolean(selectedRecommendationElement&&supported.has(selectedRecommendationElement)),ready=hasDecision&&armourValidation.ready&&hasElement&&!recommendationBusy,button=byId('generateMaxLoadout'),status=byId('recommendationReadiness');
...
function renderModRecommendation(build={}){
  ...
  const itemRows=(plan.items||[]).map(item=>`<article class="review-mod-item"><header><b>${esc(item.itemName)}</b><span>${item.projectedUsed==null?'ENERGY UNRESOLVED':`${item.projectedUsed}/${item.capacity} ENERGY`}</span></header><div>${(item.decisions||[]).map(row=>`<div class="review-mod-decision is-${String(row.action||'keep').toLowerCase()}"><em>${esc(row.action)}</em><span class="review-mod-swap">${reviewModIdentity(row.current)}<i>→</i>${reviewModIdentity(row.recommended)}</span><small>${esc(row.reasons?.[0]?.label||`${row.verifiedOptions} verified insertable option${row.verifiedOptions===1?'':'s'}; installed state retained.`)}</small></div>`).join('')||'<small>No resolved functional mod sockets.</small>'}</div></article>`).join('');
  host.innerHTML=`<div class="review-mod-summary"><div><b>RAW → CURRENT → RECOMMENDED</b><span>Forge Loader raw stats remain mod-free. Installed and proposed mod contributions are evaluated separately.</span></div><div class="review-mod-stats">${statRows}</div><small>${Number(plan.summary?.keep||0)} KEEP · ${Number(plan.summary?.replace||0)} REPLACE · ${Number(plan.summary?.add||0)} ADD · ${Number(plan.summary?.remove||0)} REMOVE</small></div><div class="review-mod-items">${itemRows}</div>${plan.limitations?.length?`<ul class="review-mod-limitations">${plan.limitations.slice(0,5).map(row=>`<li>${esc(row)}</li>`).join('')}</ul>`:''}`;
}
...
function renderRecommendedBuildReview(build={}){
  const subclass=resolvedSubclassOptions(build).find(item=>elementOf(item)===elementOf({element:build.subclass||build.subclassName||''})),superItem=build.subclassBuild?.super||build.super,abilities=build.subclassBuild?.abilities||[],aspects=build.subclassBuild?.aspects||[],fragments=build.subclassBuild?.fragments||[];
  byId('recommendedBuildSubtitle').textContent=`${String(build.characterClass||'Guardian').toUpperCase()} · ${String(build.recommendationElement||build.subclassName||build.subclass||'verified subclass').toUpperCase()} · ${String(build.objective||'balanced').toUpperCase()} · generated from the staged Forge Loader result`;
  ...
  const intelligenceHost=byId('recommendedIntelligenceSummary'),decisions=build.forgeIntelligence?.decisions||[],matched=decisions.filter(row=>row.score>0).slice(0,5),reviewRows=matched.length?matched:decisions.slice(0,5);if(intelligenceHost)intelligenceHost.innerHTML=reviewRows.map(row=>`<li>${esc(decisionReasonText(row))}</li>`).join('')||(build.forgeIntelligence?.limitations||[]).map(row=>`<li>${esc(row)}</li>`).join('')||'<li>No intelligence claim is available for this snapshot.</li>';
  ...
  const weaponSelections=new Map((build.weaponSelectionRecommendation?.decisions||[]).map(row=>[String(row.bucketHash),row]));byId('recommendedWeaponsSummary').innerHTML=Array.from({length:3},(_,index)=>{const item=build.weapons?.[index];if(!item)return '<article class="review-weapon is-unresolved"><b>WEAPON SLOT '+(index+1)+'</b><small>Verified instance unavailable</small></article>';const key=String(item.itemInstanceId||item.hash||item.bungieHash||''),row=item.weaponRollAdvice||recommendations.get(key),options=row?.best?.options||[],perks=options.map(option=>{const source=findWeaponPerk(item,option.hash);return reviewIcon(source||option,'Recommended perk');}).join(''),selection=weaponSelections.get(String(item.bucketHash));return `<article class="review-weapon"><div>${reviewIcon(item,'Weapon')}<span><b>${esc(item.name||`Weapon ${index+1}`)}</b><small>${esc(item.itemTypeDisplayName||item.weaponType||'Verified owned weapon')}</small><em>${esc(selection?.action||'KEEP')} · ${Number(selection?.candidateCount||0)} OWNED CANDIDATES</em></span></div><p>${esc(selection?.reasons?.[0]?.label||'Exact owned instance retained; no stronger explicit synergy evidence was proven.')}</p><div class="review-weapon-perks">${row?.hasVerifiedRecommendation?perks:'<small>NO CURATED PERK MATCH CLAIMED</small>'}</div></article>`;}).join('');
  const artifact=build.artifact,recommendation=build.artifactRecommendation,selected=new Set((build.artifactConfiguration?.selectedPerkHashes||recommendation?.selectedPerkHashes||[]).map(String)),perks=(artifact?.perks||[]).filter(perk=>selected.has(String(perk?.hash??perk?.itemHash??perk?.bungieHash)));
  byId('recommendedArtifactSummary').innerHTML=artifact?`<div class="review-artifact-identity">${reviewIcon(artifact,'Artifact')}<div><b>${esc(artifact.name||'VERIFIED ARTIFACT')}</b><span>${recommendation?.selectionStatus==='ready'?'PARADOX BEST VERIFIED FIT':'CURRENT VERIFIED CONFIGURATION'}</span></div></div><div class="review-artifact-perks">${perks.map(perk=>reviewIcon(perk,'Artifact perk')).join('')||'<small>NO LEGAL VERIFIED PERK SELECTION RESOLVED</small>'}</div>`:'<small>ARTIFACT STATE UNAVAILABLE</small>';
  const transfer=byId('liveTransferStatus'),apply=byId('applyBuild'),canApply=Boolean(build.recommendationGeneratedAt&&advice?.stagedChanges?.length&&advice.remotePerkMutationSupported);if(apply){apply.disabled=!canApply;apply.title=canApply?'Review the final confirmation before applying supported Bungie socket changes.':'Recommendation review only · the authenticated complete live-transfer route is not enabled.';}if(transfer)transfer.textContent=canApply?'Supported changes are staged. A final confirmation is still required.':'Live transfer locked · no complete authenticated mutation route is available. Your in-game Guardian cannot change.';
}
...
async function generateMaxLoadout(){
  ...
  const state=readState(),build=state?.workingBuild,armourValidation=validateTierFiveArmour(build||{}),candidate=resolvedSubclassOptions(build||{}).find(item=>elementOf(item)===selectedRecommendationElement);
  if(!state?.originalBuild||!build?.forgeLoaderDecision||!armourValidation.ready||!candidate){const reason=!build?.forgeLoaderDecision?'Forge Loader decision required.':armourValidation.reason||'The selected elemental build option is not supported by this Guardian’s verified subclass catalogue.';const status=byId('recommendationReadiness');if(status){status.className='recommendation-readiness is-blocked';status.textContent=reason;}return;}
  recommendationBusy=true;renderRecommendationControls(build);
  try{
    let next=protectBuildState(state),working=JSON.parse(JSON.stringify(next.workingBuild));const composed=composeForgeRecommendation({build:working,candidate,element:selectedRecommendationElement,analyzeBuild:analyzeLiveGuardian});working=composed.workingBuild;working.recommendationGeneratedAt=new Date().toISOString();working.recommendationElement=selectedRecommendationElement;working.recommendationStatus='review-required';working.forgeIntelligence={...composed.intelligence,generatedAt:working.recommendationGeneratedAt};working.paradoxAnalysis=composed.analysis||analyzeLiveGuardian(working)||null;next={...next,workingBuild:working,recommendation:{status:'review-required',generatedAt:working.recommendationGeneratedAt,element:selectedRecommendationElement,source:'verified-forge-loader-working-build',intelligenceMethod:working.forgeIntelligence.method}};
    const supplied=Number(working.currentSeasonNumber??working.currentSeason?.seasonNumber),currentSeasonNumber=Number.isInteger(supplied)?supplied:await fetchCurrentArtifactSeason(),artifactResult=applyForgeArtifactRecommendation(next,{currentSeasonNumber,force:true});next=artifactResult.state;working=JSON.parse(JSON.stringify(next.workingBuild));working.objective=selectedRecommendationObjective;const weaponResult=selectOwnedWeapons({build:working,objective:selectedRecommendationObjective});working=weaponResult.workingBuild;working.paradoxAnalysis=analyzeLiveGuardian(working)||working.paradoxAnalysis||null;const modResult=recommendArmourMods({build:working,objective:selectedRecommendationObjective});working=modResult.workingBuild;working.paradoxAnalysis=analyzeLiveGuardian(working)||working.paradoxAnalysis||null;await adviseLiveWeaponRolls(working,working.paradoxAnalysis||{}, {insertSocketPlugFree:false});if(working.forgeIntelligence&&working.paradoxAnalysis){working.forgeIntelligence.evidence={...working.forgeIntelligence.evidence,directedLinks:working.paradoxAnalysis.buildLoop?.length||0,strengths:working.paradoxAnalysis.strengths?.length||0,weakLinks:working.paradoxAnalysis.weakLinks?.length||0,confidence:working.paradoxAnalysis.confidence?.level||'evidence-limited',ownedWeaponCandidates:working.weaponSelectionRecommendation?.candidateCount||0,armourModDecisions:working.armourModRecommendation?.decisions?.length||0};working.forgeIntelligence.limitations=[...new Set([...(working.forgeIntelligence.limitations||[]),...(working.weaponSelectionRecommendation?.limitations||[]),...(working.armourModRecommendation?.limitations||[])])];}next=protectBuildState({...next,workingBuild:working});writeState(next);render();openRecommendedBuild();
  ...
  finally{recommendationBusy=false;renderRecommendationControls(currentBuild()||{});}
}
...
function renderBuildSurface(){
  ...
  const automaticArtifact=recommendation?.selectionStatus==='ready'&&!recommendation?.userOverride;byId('artifactStatus').textContent=artifactUnavailable?'ARTIFACT STATE UNAVAILABLE':(automaticArtifact?'PARADOX ARTIFACT 2.0 FIT STAGED':(activePerks.length?'LIVE ARTIFACT RESOLVED':'NO ARTIFACT FIT STAGED'));byId('artifactStatusDetail').textContent=artifactUnavailable?'Verified Artifact socket data is unavailable.':automaticArtifact?`${artifact.name||'Artifact'} · best of ${recommendation.artifactCandidateCount||1} verified Artifact option(s) · ${recommendation.selectionLimit} socket-bucket picks staged for this Working Build. Live Guardian unchanged.`:`${artifact.name||'Artifact'} · ${activePerks.length} active perk(s) captured into this build snapshot.`;
  ...
  const advice=build.weaponRollAdvice||build.paradoxAnalysis?.weaponRollAdvice,apply=byId('applyBuild'),canApply=Boolean(build.recommendationGeneratedAt&&advice?.stagedChanges?.length&&advice.remotePerkMutationSupported);if(apply){apply.disabled=!canApply;apply.title=canApply?'Review and confirm the supported staged Bungie changes.':'Build My Guardian stays locked until a generated recommendation has a complete authenticated action route.';}renderRecommendationControls(build);renderTestConfiguration();refreshRangeCapture();
...
async function applyBuild(){const build=currentBuild(),advice=build?.weaponRollAdvice||build?.paradoxAnalysis?.weaponRollAdvice;const plan=createPerkChangePlan({characterId:build?.characterId,advice});try{if(!build?.recommendationGeneratedAt)throw new Error('Generate and review a verified Working Build before any live action.');if(!plan.remotePerkMutationSupported)throw new Error('The recommendation is staged, but the authenticated Bungie socket-action route is not enabled.');if(!window.confirm(`BUILD MY GUARDIAN LOADOUT will apply ${plan.changes.length} supported verified perk change${plan.changes.length===1?'':'s'} to the live Guardian. Armour, mods and unsupported changes remain untouched. Continue?`))return;const confirmed=confirmPerkChangePlan(plan);setRangeStatus('Applying explicitly confirmed reversible perk changes…','warn');const result=await applyConfirmedPerkChangePlan(confirmed);showRangeOutput(result);setRangeStatus('Confirmed perk changes applied. Refreshing Bungie evidence is required.','good');}catch(error){setRangeStatus(error?.message||'Unable to apply the staged build.','bad');}}
