...
import {composeForgeRecommendationAsync,filterExoticCompatibleSubclasses,hasVerifiedSubclassSockets,synchroniseSubclassProjection} from './paradox-forge-intelligence.mjs?v=20260905-forge-responsive-1';
...
function deferPanel(id,draw){deferredPanels.set(id,draw);if(!byId(id)?.hidden)draw();}
...
function artifactRecommendationMarkup(build){
  ...
  if(!recommendation)return '<div class="artifact-recommendation is-neutral"><b>CURRENT ARTIFACT RETAINED</b><small>Checking the active Bungie season and legal perk matrix.</small></div>';
...
async function fetchCurrentArtifactSeason(){
  ...
  if(result===null&&artifactSeasonRequest===pending)artifactSeasonRequest=null;
  return result;
}
...
function renderRecommendationControls(build={}){
  ...
  if(status){status.className='recommendation-readiness'+(ready?' is-ready':' is-blocked');status.textContent=recommendationBusy?'Choose an element and generate to compare verified build options.':!hasDecision?'Stage a verified Forge Loader armour result to unlock compatible build options.':!armourValidation.ready?armourValidation.reason:!exoticValidation.ready?exoticValidation.reason:!hasElement?'Select an available verified elemental build option.':`Ready · ${selectedRecommendationElement.toUpperCase()} damage build · one verified Exotic armour anchor · Maximized Forge Loader result.`;}
}
...
// The live analyser consumes equipped components, not alternate socket or inventory catalogues.
const FORGE_ANALYSIS_FIELDS=Object.freeze(['source','characterId','selectedLoadoutIndex','super','classAbility','movement','melee','grenade','aspects','fragments','artifact','weapons','paradoxEvidence','hashCoverage','statModel','artifactValidation','activityProfile','activity','beta','buildFocus']);
function analyzeForgeCandidate(build){return analyzeLiveGuardian(Object.fromEntries(FORGE_ANALYSIS_FIELDS.filter(key=>Object.hasOwn(build,key)).map(key=>[key,build[key]])));}
...
async function generateMaxLoadout(){
  ...
  try{
    ...
    const composed=await composeForgeRecommendationAsync({build:forgeComputationProjection(working),candidate,element:selectedRecommendationElement,analyzeBuild:analyzeForgeCandidate});
    if(readState()!==state)throw new Error('The selected build changed during generation. Generate again for the current build.');
...
function renderBuildSurface(){
  ...
  deferPanel('abilityOptions',()=>{byId('abilityOptions').innerHTML=abilityGroups(build,abilities);});
  for(const [kind,nodeId] of [['aspects','aspectOptions'],['fragments','fragmentOptions']])deferPanel(nodeId,()=>{const equipped=kind==='aspects'?aspects:fragments,options=resolvedOptions(build,kind);byId(nodeId).innerHTML=options.length?options.map((item,index)=>selectorCard(item,{kind,index,selected:equipped.some(value=>itemKey(value)===itemKey(item))})).join(''):unavailableCard('Compatible '+kind+' unavailable');});
  }catch(error){
...
document.addEventListener('click',event=>{const recommendationElement=event.target.closest('[data-recommendation-element]');if(recommendationElement&&!recommendationElement.disabled&&!recommendationBusy){selectedRecommendationElement=recommendationElement.dataset.recommendationElement||'';renderRecommendationControls(currentBuild()||{});return;}const objective=event.target.closest('[data-build-objective]');if(objective&&!objective.disabled&&!recommendationBusy){selectedRecommendationObjective=objective.dataset.buildObjective||'balanced';renderRecommendationControls(currentBuild()||{});return;}const artifactRecommend=event.target.closest('[data-artifact-recommend]');if(artifactRecommend){artifactRecommend.disabled=true;void refreshForgeArtifactRecommendation({force:true});return;}const candidate=event.target.closest('[data-confirm-instance]');if(candidate){try{const confirmed=confirmCandidateActivity(candidate.dataset.confirmInstance,{expectedCharacterId:currentBuild()?.characterId});refreshRangeCapture();showRangeOutput(confirmed);setRangeStatus(`COMPLETED ACTIVITY CONFIRMED · ${candidate.dataset.confirmInstance}`,'good');}catch(error){setRangeStatus(error?.message||'Unable to confirm this activity.','bad');}return;}const toggle=event.target.closest('[data-toggle-panel]');if(toggle){const panel=byId(toggle.dataset.togglePanel),expanded=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!expanded));if(panel){panel.hidden=expanded;if(!expanded)deferredPanels.get(panel.id)?.();}return;}const option=event.target.closest('[data-select-kind]');if(option){const kind=option.dataset.selectKind;stageSelection(kind,Number(option.dataset.selectIndex));}});
...
async function initialiseBuildForge(){
  try{
    ...
    if(initialVaultState){const nextVaultState=atomicTransfer||applyPendingVaultSelection(initialVaultState);if(nextVaultState!==initialVaultState)writeState(nextVaultState);}
  }catch(error){
