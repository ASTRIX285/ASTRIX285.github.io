...
import {createLiveTransferPreflight,deriveLoadoutIntent,recommendArmourMods,selectOwnedWeapons,validateArmourModLoadout,validateExoticLoadout,validateLoadoutCoherence} from './paradox-loadout-intelligence.mjs?v=20260904-cross-system-loop-1';
...
function renderRecommendedBuildReview(build={}){
  ...
  const transfer=byId('liveTransferStatus'),apply=byId('applyBuild'),preflight=build.liveTransferPreflight||null,canApply=Boolean(preflight?.ready&&build.recommendationGeneratedAt&&advice?.stagedChanges?.length&&advice.remotePerkMutationSupported);if(apply){apply.disabled=!canApply;apply.title=canApply?'Review the final confirmation before applying supported Bungie socket changes.':preflight?.ready?'Full recommendation preflight passed; the authenticated complete live-transfer route is not enabled.':'Recommendation transfer preflight is blocked.';}if(transfer)transfer.textContent=canApply?'Supported changes and full-build preflight are ready. A final confirmation is still required.':preflight?.ready?'Live-session transfer preflight passed · authenticated full-loadout mutation is not available, so the in-game Guardian remains unchanged.':`Live transfer blocked · ${preflight?.violations?.[0]||'the generated build has not passed exact-loadout preflight.'}`;
}
...
async function applyBuild(){const build=currentBuild(),advice=build?.weaponRollAdvice||build?.paradoxAnalysis?.weaponRollAdvice,preflight=createLiveTransferPreflight(build||{}),plan=createPerkChangePlan({characterId:build?.characterId,advice});try{if(!build?.recommendationGeneratedAt)throw new Error('Generate and review a verified Working Build before any live action.');if(!preflight.ready)throw new Error(preflight.violations[0]||'The exact Working Build failed live-transfer preflight.');if(!plan.remotePerkMutationSupported)throw new Error('The full recommendation passed preflight, but the authenticated Bungie socket-action route is not enabled.');if(!window.confirm(`BUILD MY GUARDIAN LOADOUT will apply ${plan.changes.length} supported verified perk change${plan.changes.length===1?'':'s'} to the live Guardian. Armour, mods and unsupported changes remain untouched. Continue?`))return;const confirmed=confirmPerkChangePlan(plan);setRangeStatus('Applying explicitly confirmed reversible perk changes…','warn');const result=await applyConfirmedPerkChangePlan(confirmed);showRangeOutput(result);setRangeStatus('Confirmed perk changes applied. Refreshing Bungie evidence is required.','good');}catch(error){setRangeStatus(error?.message||'Unable to apply the staged build.','bad');}}
