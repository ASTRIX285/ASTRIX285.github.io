...
function artifactRecommendationMarkup(build){
  ...
  return '<div class="artifact-recommendation is-ready"><div><b>PARADOX BEST VERIFIED FIT</b><span>'+Number(recommendation.selectedMatchedCount||0)+' OF '+Number(recommendation.selectionLimit||0)+' PICKS DIRECTLY MATCH THE STAGED BUILD</span></div>'+(reasons?'<ol>'+reasons+'</ol>':'')+'<small>Ranked from verified descriptions and the staged Exotic, stat priorities, armour-set traits, subclass/Super and weapons. Working Build only · live Guardian unchanged.</small><button type="button" data-artifact-recommend>RECALCULATE BEST FIT</button></div>';
}
function artifactMatrix(artifact,configuration,recommendation){
  const perks=Array.isArray(artifact?.perks)?artifact.perks:[];
  if(!perks.length)return '<div class="artifact-spectrum-unavailable"><b>FULL PERK SPECTRUM UNRESOLVED</b><small>Requires Bungie CharacterProgressions component 202 seasonalArtifact.tiers[].items plus manifest item definitions.</small></div>';
  ...
  return [...tiers.entries()].sort((a,b)=>a[0]-b[0]).map(([tier,rows])=>{const requirement=Number(rows[0]?.perk?.minimumUnlockPointsUsedRequirement??rows[0]?.perk?.pointsToUnlock),tierTitle=esc(rows[0]?.perk?.tierTitle||('TIER '+(tier+1)));return '<section class="artifact-tier" data-artifact-tier="'+tier+'"><h4>'+tierTitle+(Number.isFinite(requirement)&&requirement>0?' · '+requirement+' PRIOR PICKS':'')+'</h4><div class="artifact-tier-perks">'+rows.sort((a,b)=>(a.perk.itemIndex||0)-(b.perk.itemIndex||0)).map(row=>{const key=String(row.perk?.hash),detail=ranked.get(key);return artifactPerkCard(row.perk,row.index,{selected:selected.has(key),recommended:automatic&&selected.has(key),recommendation:detail});}).join('')+'</div></section>';}).join('');
}
