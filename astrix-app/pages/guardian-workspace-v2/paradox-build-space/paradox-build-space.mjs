...
import {BUILD_ELEMENTS,armourTierOf,verifiedMasterworkState,validateTierFiveArmour} from './paradox-build-recommendation.mjs';
...
function decorateBuildArmour(build={}){document.querySelectorAll('#armourGrid .gear-slot').forEach((card,index)=>{const item=build.armour?.[index],tier=armourTierOf(item),line=document.createElement('div');line.className='armour-verification-line';line.innerHTML=`<span>${Number.isFinite(tier)?`T${esc(tier)} VERIFIED`:'TIER UNRESOLVED'}</span><span>${esc(verifiedMasterworkState(item))}</span>`;card.append(line);});}
...
function renderRecommendationControls(build={}){
  ...
  if(status){status.className='recommendation-readiness'+(ready?' is-ready':' is-blocked');status.textContent=recommendationBusy?'Resolving subclass, weapon and Artifact evidence…':!hasDecision?'Forge Loader decision required.':!armourValidation.ready?armourValidation.reason:!hasElement?'Choose an available verified subclass.':`Ready · ${selectedRecommendationElement.toUpperCase()} · five T5 armour instances · Maximized Forge Loader result.`;}
}
...
function renderRecommendedBuildReview(build={}){
  ...
  byId('recommendedBuildSubtitle').textContent=`${String(build.characterClass||'Guardian').toUpperCase()} · ${String(build.recommendationElement||build.subclassName||build.subclass||'verified subclass').toUpperCase()} · generated from the staged Forge Loader result`;
  ...
  byId('recommendedWeaponsSummary').innerHTML=Array.from({length:3},(_,index)=>{const item=build.weapons?.[index];if(!item)return '<article class="review-weapon is-unresolved"><b>WEAPON SLOT '+(index+1)+'</b><small>Verified instance unavailable</small></article>';const key=String(item.itemInstanceId||item.hash||item.bungieHash||''),row=item.weaponRollAdvice||recommendations.get(key),options=row?.best?.options||[],perks=options.map(option=>{const source=findWeaponPerk(item,option.hash);return reviewIcon(source||option,'Recommended perk');}).join('');return `<article class="review-weapon"><div>${reviewIcon(item,'Weapon')}<span><b>${esc(item.name||`Weapon ${index+1}`)}</b><small>${esc(item.itemTypeDisplayName||item.weaponType||'Verified owned weapon')}</small></span></div><div class="review-weapon-perks">${row?.hasVerifiedRecommendation?perks:'<small>NO CURATED PERK MATCH CLAIMED</small>'}</div></article>`;}).join('');
...
async function generateMaxLoadout(){
  ...
  if(!state?.originalBuild||!build?.forgeLoaderDecision||!armourValidation.ready||!candidate){const reason=!build?.forgeLoaderDecision?'Forge Loader decision required.':armourValidation.reason||'The selected subclass is not verified for this Guardian.';const status=byId('recommendationReadiness');if(status){status.className='recommendation-readiness is-blocked';status.textContent=reason;}return;}
  recommendationBusy=true;renderRecommendationControls(build);
  try{
    let next=protectBuildState(state),working=JSON.parse(JSON.stringify(next.workingBuild));applySubclassCandidate(working,candidate);working.recommendationGeneratedAt=new Date().toISOString();working.recommendationElement=selectedRecommendationElement;working.recommendationStatus='review-required';working.paradoxAnalysis=analyzeLiveGuardian(working)||null;await adviseLiveWeaponRolls(working,working.paradoxAnalysis||{}, {insertSocketPlugFree:false});next={...next,workingBuild:working,recommendation:{status:'review-required',generatedAt:working.recommendationGeneratedAt,element:selectedRecommendationElement,source:'verified-forge-loader-working-build'}};
    const supplied=Number(working.currentSeasonNumber??working.currentSeason?.seasonNumber),currentSeasonNumber=Number.isInteger(supplied)?supplied:await fetchCurrentArtifactSeason(),artifactResult=applyForgeArtifactRecommendation(next,{currentSeasonNumber,force:true});next=artifactResult.state;writeState(next);render();openRecommendedBuild();
