...
function semanticText(item){
  return [
    ...
    item?.definition?.plug?.plugCategoryIdentifier,
    ...(item?.definition?.traitIds||[])
  ].filter(Boolean).join(" ").toLowerCase();
}

function plugCategory(item){
  return norm(item?.definition?.plug?.plugCategoryIdentifier);
}

function classifyArmourPlug(plug){
  const category=plugCategory(plug);
  const text=semanticText(plug);
...
function setEffects(plugs){
  ...
  return {
    identity:null,
    twoPiece:effects.find(plug=>/\b2[ -]?piece\b/.test(semanticText(plug)))||null,
    fourPiece:effects.find(plug=>/\b4[ -]?piece\b/.test(semanticText(plug)))||null,
    effects
  };
}
...
function normaliseArmourSemantics({plugs=[],instance=null,stats=null}={}){
  ...
  return {
    tier,
    masterwork:buckets.masterwork[0]||null,
    masterworkCandidates:uniq(buckets.masterwork),
    energy:energy?{
      type:energy.energyType??null,
      typeHash:energy.energyTypeHash??null,
      capacity:Number.isFinite(Number(energy.energyCapacity))?Number(energy.energyCapacity):null,
      used:Number.isFinite(Number(energy.energyUsed))?Number(energy.energyUsed):null
    }:null,
    archetype:buckets.archetype[0]||null,
    generalMods:uniq(buckets.generalMods),
    slotMods:uniq(buckets.slotMods),
    exoticPerk:buckets.exoticPerk[0]||null,
    set:setEffects(plugs),
    stats:stats?.stats||stats||null,
    discarded:uniq(buckets.discarded),
    unknownPlugs:uniq(buckets.unknown),
    complete:buckets.unknown.length===0
  };
}
...
function catalystProgress(profile,itemInstanceId,catalyst){
  ...
  return {
    ...
    objectives,
    completed,
    active:Boolean(catalyst.isEnabled)&&completed
  };
}

function normaliseWeaponSemantics({profile=null,item=null,plugs=[],instance=null,stats=null}={}){
  ...
  return {
    intrinsic:groups.intrinsic[0]||null,
    selectedPerks:uniq(groups.perks),
    masterwork:groups.masterwork[0]||null,
    mod:groups.mod[0]||null,
    catalyst:catalyst?{...catalyst,progress:catalystProgress(profile,item?.itemInstanceId,catalyst)}:null,
    champion:instance?.breakerTypeHash||instance?.breakerType?{
      breakerType:instance.breakerType??null,
      breakerTypeHash:instance.breakerTypeHash??null,
      source:"bungie-item-instance"
    }:null,
    stats:stats?.stats||stats||null,
    unknownPlugs:uniq(groups.unknown),
    complete:groups.unknown.length===0
  };
}

function normaliseGuardianStats(statPairs=[]){
  return Object.fromEntries((statPairs||[]).map(([name,value])=>{
    const numeric=Number(value)||0;
    return [name,{value:numeric,enhancedThreshold:100,enhancedThresholdReached:numeric>100}];
  }));
}

function validateArtifact(artifact){
  const active=artifact?.activePerks||[];
  const hashes=active.map(item=>Number(item?.hash)).filter(Number.isFinite);
...
export {semanticText,plugCategory,classifyArmourPlug,normaliseArmourSemantics,classifyWeaponPlug,normaliseWeaponSemantics,normaliseGuardianStats,validateArtifact};
