...
if(new URLSearchParams(location.search).has('rangeTest')){
  ...
  </style><h3>PARADOX SHOOTING RANGE CAPTURE</h3><p>This runs inside the live Guardian Workspace. Select the Guardian here first, then arm the test.</p><div class="rr"><button class="primary" data-act="arm">ARM TEST</button><button data-act="pull">PULL RESULTS</button><button data-act="clear">CLEAR</button><button data-act="close">CLOSE</button></div><pre>Ready. Select the Guardian in the cards above, then press ARM TEST.</pre>`;
