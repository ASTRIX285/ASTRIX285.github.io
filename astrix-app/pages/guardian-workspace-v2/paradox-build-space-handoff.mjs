...
function rememberGuardian(detail={}){
  ...
  activeCharacterId=String(detail.characterId);
...
function rememberArtifactSelection(detail={}){for(const build of [latestGuardian,latestExplicitLoadout]){if(!matchesArtifactSelection(build,detail))continue;build.artifactConfiguration=clone(detail.artifactConfiguration||null);if(detail.artifact)build.artifact={...clone(build.artifact||{}),...clone(detail.artifact),state:detail.state||build.artifact?.state,activePerks:activePerksFromArtifactEvent(detail,build),artifactConfiguration:clone(detail.artifactConfiguration||null)};}if(matchesArtifactSelection(latestExplicitLoadout,detail))safeStore(LAST_LOADOUT_KEY,latestExplicitLoadout);}
...
export {compactBuild,rememberGuardian,rememberExplicitLoadout,rememberArtifactSelection,resolveBuildSource,BUILD_SPACE_KEY,LAST_LOADOUT_KEY};
