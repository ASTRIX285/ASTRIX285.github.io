...
function renderWeapons(weapons=[]){
  ...
  cards.forEach((card,index)=>{
    ...
    if(!weapon){
      if(art){art.classList.add("ph");art.innerHTML='<span class="pw">—</span><span class="ph-glyph">⌖</span>'}
      if(meta)meta.textContent="awaiting build data";
      return;
    }
    if(art){
      art.classList.remove("ph");
      const icon=weapon.icon?`<img src="${escapeHtml(weapon.icon)}" alt="${escapeHtml(weapon.name||"Weapon")}" onerror="this.style.display='none'">`:'<span class="ph-glyph">⌖</span>';
      art.innerHTML=`<span class="pw">${escapeHtml(String(weapon.power??""))}</span>${icon}`;
    }
    if(name)name.textContent=weapon.name||"Unknown weapon";
    if(meta){const details=[weapon.weaponType,weapon.element,weapon.ammoType].filter(Boolean);meta.textContent=details.join(" · ")||"Bungie identity resolved"}
...
function renderArmourFunctionalSlots(armour=[]){
  ...
  cards.forEach((card,index)=>{
    ...
    if(isExotic){
      ...
      if(trait.icon){
        const img=document.createElement("img");img.src=trait.icon;img.alt=name;img.onerror=()=>{img.style.display="none"};traitButton.appendChild(img);
      }else traitButton.textContent="✦";
    ...
    for(let i=0;i<slotCount;i+=1){const slot=document.createElement("div");slot.className="pf-mod-slot";slot.title=`Armour mod slot ${i+1}`;slot.innerHTML='<span aria-hidden="true">◇</span>';grid.appendChild(slot)}
