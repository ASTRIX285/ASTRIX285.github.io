...
function manifestIdentity(hash){
  const h=Number(hash),row=manifestByHash.get(String(h)); if(!row)return null;
...
const resolve=hash=>{
  const h=Number(hash),legacy=byHash.get(String(h))??null,official=manifestIdentity(h);
  if(legacy&&official)return withTraitDirectionEvidence({...legacy,...official,sourceKind:legacy.sourceKind??official.sourceKind},h);
  if(official)return withTraitDirectionEvidence(official,h); if(legacy)return withTraitDirectionEvidence(legacy,h);
...
function enrichArmourItem(item,fixture,displayMods=[]){
  const official=manifestByHash.get(String(item.hash))??null,bucketHash=official?.equippingBlock?.equipmentSlotTypeHash??item.equipmentSlotTypeHash??null,appearanceByBucket=fixture.rawDim?.parameters?.modsByBucket??{},appearanceHashes=bucketHash!=null?(appearanceByBucket[String(bucketHash)]??[]):[],rarityText=String(item.rarity??item.tier??"").toLowerCase(),isExotic=item.isExotic===true||rarityText.includes("exotic")||Boolean(official?.equippingBlock?.uniqueLabelHash),intrinsicHashes=isExotic?(official?.intrinsicPlugHashes??[]):[],intrinsicTraits=intrinsicHashes.map(resolve).filter(Boolean);
