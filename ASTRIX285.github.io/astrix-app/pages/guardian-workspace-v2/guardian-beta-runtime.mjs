/* ==========================================================================
   ...
   stat bars, and OAuth return-to-Forge lifecycle.
   ========================================================================== */
...
/* ==========================================================================
   BUNGIE AUTHENTICATION & REDIRECT PRESERVATION
   ========================================================================== */

export function startBungieAuth() {
  const currentWorkspaceUrl = window.location.href;
  window.location.href = `${AUTH_ORIGIN}/bungie/start?return=${encodeURIComponent(currentWorkspaceUrl)}`;
}

function handleIncomingAuthCode() {
  const params = new URLSearchParams(window.location.search);
  const authCode = params.get("code");
  if (authCode) {
    console.log("OAuth authorization code detected on Workspace V2:", authCode);
    setStageState("BUNGIE CONNECTED", "Exchanging authorization code with Paradox edge worker...");
    window.history.replaceState({}, document.title, window.location.pathname + (params.has("verify") ? `?verify=${params.get("verify")}` : ""));
  }
}

function setupAuthHooks() {
  const btnTop = byId("btnConnectBungie");
  const bannerStatus = document.querySelector(".guardian-character-cards__status");

  if (btnTop) btnTop.addEventListener("click", startBungieAuth);
  if (bannerStatus) {
    bannerStatus.style.cursor = "pointer";
    bannerStatus.addEventListener("click", startBungieAuth);
  }
}
