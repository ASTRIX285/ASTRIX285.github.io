...
export class GameComponentService {
  ...
  async load(options = {}) {
    ...
    if (!response.ok) {
      throw new Error(
        `Unable to load game components: `
        + `${response.status}`
      );
    }

    return this.setCatalogue(
      await response.json()
    );
  }
  ...
  setCatalogue(catalogue) {
    if (
      ...
      throw new Error(
        'Invalid game-component catalogue.'
      );
    }
  ...
  buildIndexes() {
    ...
    for (
      const component
      of this.catalogue.components
    ) {
      this.byHash.set(
        String(
          component.bungieHash
        ),
        component
      );
...
export const gameComponentService =
  new GameComponentService();
