...
const added=[];const newlySymmetric=[];const newDuplicates=[];const rows=[];
for(const f of fixtures.fixtures??[]){
 ...
 rows.push({fixtureId:f.fixtureId,before:{confidence:b.confidence.level,count:b.buildLoop.length},after:{confidence:c.confidence.level,count:c.buildLoop.length},added:addedHere.map(l=>({chain:l.chain,source:l.source})),traitLinks:c.buildLoop.filter(l=>String(l.source).includes('runtime-traitid-parsing')).map(l=>({chain:l.chain,source:l.source,evidence:l.evidence})),candidates:c.candidateRelationships??[]});
}
