# ASTRIX backend and Bungie OAuth design
...
| GET | `/v1/me` | Returns `501 OAuth gate closed` | Return minimal authenticated ASTRIX profile |
...
9. The Worker creates an ASTRIX session record that references the server-side Bungie token record.
10. The Worker sends the browser a short-lived opaque session cookie and redirects to an approved frontend return path.
11. Authenticated API calls send only the ASTRIX session cookie. The Worker resolves the server-side session and uses the server-held Bungie access token when needed.
...
The browser holds only an opaque ASTRIX session identifier in a cookie. It must not hold the Bungie access token, Bungie refresh token or client secret.
...
2. revoke the server-side ASTRIX session;
...
- [ ] Sign in to or create the Cloudflare account that will own ASTRIX production infrastructure.
...
- [ ] From `astrix-worker/`, run `npm install`.
